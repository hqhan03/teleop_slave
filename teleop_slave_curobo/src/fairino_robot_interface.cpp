...
errno_t FairinoRobotSdkAdapter::MoveJ(JointPos* joint_pos, int tool, int user, float vel,
                                      float acc, float ovl, ExaxisPos* epos, float blend_t,
                                      uint8_t offset_flag, DescPose* offset_pos) {
    return robot_.MoveJ(joint_pos, tool, user, vel, acc, ovl, epos, blend_t, offset_flag,
                        offset_pos);
}
