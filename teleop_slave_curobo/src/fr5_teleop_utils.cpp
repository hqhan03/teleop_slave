...
ServoCartMode ParseServoCartMode(const std::string& mode) {
    if (mode == "incremental_base") {
        return ServoCartMode::kIncrementalBase;
    }
    if (mode == "absolute_base") {
        return ServoCartMode::kAbsoluteBase;
    }

    throw std::invalid_argument(
        "Unsupported servocart_mode: " + mode +
        " (expected 'incremental_base' or 'absolute_base')");
}

std::string ServoCartModeToString(ServoCartMode mode) {
    switch (mode) {
        case ServoCartMode::kIncrementalBase:
            return "incremental_base";
        case ServoCartMode::kAbsoluteBase:
...
geometry_msgs::msg::Pose BuildServoCartCommandPose(
    ...
    const geometry_msgs::msg::Pose normalized_target =
        NormalizePoseQuaternion(sampled_target_pose);

    switch (mode) {
        case ServoCartMode::kIncrementalBase:
            if (!previous_command_pose_valid) {
                return MakeIdentityPose();
            }
            return ComputeIncrementalPoseDeltaBaseFrame(
                previous_command_pose, normalized_target);
        case ServoCartMode::kAbsoluteBase:
...
double PoseTranslationDistanceMillimeters(const geometry_msgs::msg::Pose& from,
    ...
    return delta.length() * kMmPerMeter;
}
...
bool IsLikelyJointBranchJump(const geometry_msgs::msg::Pose& previous_pose,
    ...
    if (PoseTranslationDistanceMillimeters(previous_pose, current_pose) >
    ...
    tf2::fromMsg(previous_pose.orientation, previous_q);
    tf2::fromMsg(current_pose.orientation, current_q);
    if (QuaternionAngularDistanceDegrees(previous_q, current_q) >
        max_orientation_step_deg) {
        return false;
    }

    for (size_t i = 0; i < min_joint_jump_deg.size(); ++i) {
        if (min_joint_jump_deg[i] <= 0.0) {
            continue;
        }
        if (std::abs(candidate_joint_target_deg[i] - previous_joint_target_deg[i]) >=
            min_joint_jump_deg[i]) {
            return true;
        }
    }
...
PoseIkSolveResult SolvePoseTargetIK(IFairinoRobot& robot,
    ...
    if (options.has_reference_joints) {
        for (size_t i = 0; i < options.reference_joint_deg.size(); ++i) {
            reference_joints.jPos[i] = options.reference_joint_deg[i];
        }
    } else {
