...
FairinoSlaveNode::FairinoSlaveNode() : Node("fairino_slave_node") {
    ...
    this->declare_parameter("enable_kalman_position_filter", true);
    this->declare_parameter("kalman_process_noise", 0.5);
    this->declare_parameter("kalman_measurement_noise", 2.5e-5);
...
void FairinoSlaveNode::logCalibrationConfiguration() const {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Input smoothing: kalman_position=%s process_noise=%.6f measurement_noise=%.6f "
                "butterworth_cutoff_hz=%.2f sample_rate_hz=%.2f orientation_alpha=%.2f",
                this->get_parameter("enable_kalman_position_filter").as_bool() ? "enabled" : "disabled",
                this->get_parameter("kalman_process_noise").as_double(),
                this->get_parameter("kalman_measurement_noise").as_double(),
                this->get_parameter("filter_cutoff_hz").as_double(),
                this->get_parameter("filter_sample_rate_hz").as_double(),
                this->get_parameter("orientation_smoothing_alpha").as_double());
}
...
geometry_msgs::msg::PoseStamped FairinoSlaveNode::buildTargetPose(
    const geometry_msgs::msg::PoseStamped& manus_pose,
    const tf2::Vector3& tracker_position) const {
    const tf2::Vector3 delta_position = tracker_position - clutch_tracker_position_;
...
void FairinoSlaveNode::manusPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    ...
    const tf2::Vector3 raw_tracker_position(
        ...
        msg->pose.position.y,
        msg->pose.position.z);
    const tf2::Vector3 tracker_measurement = teleop_slave::ScaleVector(
        teleop_slave::ApplyAxisMapping(raw_tracker_position, tracker_to_robot_axes_, tracker_to_robot_signs_),
        translation_scale_xyz_);
    const bool tracker_stamp_is_zero =
        msg->header.stamp.sec == 0 && msg->header.stamp.nanosec == 0;
    const rclcpp::Time tracker_sample_time =
        tracker_stamp_is_zero ? this->now() : rclcpp::Time(msg->header.stamp);
    ...
    if (offset_set_) {
        ...
        clutch_tracker_position_ = tracker_measurement;
        ...
        kalman_configured_ = this->get_parameter("enable_kalman_position_filter").as_bool();
        if (kalman_configured_) {
            const double process_noise = this->get_parameter("kalman_process_noise").as_double();
            const double measurement_noise = this->get_parameter("kalman_measurement_noise").as_double();
            for (auto& f : kalman_position_filters_) {
                f.configure(process_noise, measurement_noise);
            }
            kalman_position_filters_[0].reset(clutch_tracker_position_.x());
            kalman_position_filters_[1].reset(clutch_tracker_position_.y());
            kalman_position_filters_[2].reset(clutch_tracker_position_.z());
            last_tracker_sample_time_ = tracker_sample_time;
        }
    ...
    tf2::Vector3 filtered_tracker_position = tracker_measurement;
    if (kalman_configured_) {
        const double sample_rate = this->get_parameter("filter_sample_rate_hz").as_double();
        const double nominal_dt = 1.0 / std::max(sample_rate, 1.0);
        double dt = nominal_dt;
        if (last_tracker_sample_time_.nanoseconds() != 0) {
            const double raw_dt = (tracker_sample_time - last_tracker_sample_time_).seconds();
            if (std::isfinite(raw_dt) && raw_dt > 1e-4 && raw_dt < 0.25) {
                dt = raw_dt;
            }
        }
        filtered_tracker_position.setValue(
            kalman_position_filters_[0].filter(tracker_measurement.x(), dt),
            kalman_position_filters_[1].filter(tracker_measurement.y(), dt),
            kalman_position_filters_[2].filter(tracker_measurement.z(), dt));
        last_tracker_sample_time_ = tracker_sample_time;
    }

    geometry_msgs::msg::PoseStamped target = buildTargetPose(*msg, filtered_tracker_position);
    ...
    stream_client_->async_send_request(
        ...
        [this](rclcpp::Client<std_srvs::srv::SetBool>::SharedFuture future) {
            ...
            if (response->success) {
                ...
                kalman_configured_ = false;
                last_tracker_sample_time_ = rclcpp::Time(0, 0, RCL_ROS_TIME);
